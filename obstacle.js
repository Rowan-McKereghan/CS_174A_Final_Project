...
export class Obstacle {
    ...
    draw(context, program_state) {
      if (!this.is_fractured) {
        ...
        for (let i = 0; i < this.fragments.length; i++) this.fragments[i].draw(context, program_state);
      }
    }
}
...
export class Obstacle_Fragment {
  ...
  constructor(left, bottom, front, width, height, vel_x, vel_y) {
    this.cube = new defs.Cube();
    this.material = new Material(new defs.Spotlight_Shader(), { color: color(1, 1, 1, 1), ambient: 1});

    this.pos = {x: left, y: bottom, z: front};
    this.vel = {x: vel_x, y: vel_y, z: -1};
  ...
  draw(context, program_state) {
    let transform = Mat4.identity().times(Mat4.translation(this.pos.x-this.width/2, this.pos.y-this.height/2, this.pos.z))
